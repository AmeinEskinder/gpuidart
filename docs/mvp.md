...
- Clean Windows: the `bb6a894` candidate passed in a fresh Sandbox with vGPU disabled, no developer SDK commands and PerMonitorV2 at DPI 120. [The investigation](../reports/release/sandbox-investigation/README.md) retains the earlier guest compositor crashes and the report-text encoding defect. Human IME remains pending despite installed Japanese input on the host.
