...
[Release evidence](../reports/release/README.md) records a clean Windows launch in Sandbox after the restart. [Windows Japanese IME observation](../reports/ime/windows-japanese-20260926/README.md) passed at 125% scale using real composition and screenshots, including reload while composing. The owner authorized agent observation; no independent human observer participated. Other input backends and mixed-monitor DPI movement remain unverified. Use the [observation checklist](windows-release-checks.md) to record those results. The [reload observation](../reports/mvp/attempt-023eef4/README.md) also remains unlocalized despite subsequent passing checks. The SDK uses the [MIT License](../LICENSE); dependencies retain their own terms.
