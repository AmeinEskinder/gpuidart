...
[Release evidence](../reports/release/README.md) records a clean Windows launch in Sandbox after the restart. Japanese typing is installed and enabled; actual IME composition and mixed-monitor DPI movement remain unverified. Use the [manual checks](windows-release-checks.md) to record those observations. The [reload observation](../reports/mvp/attempt-023eef4/README.md) also remains unlocalized despite subsequent passing checks. The SDK uses the [MIT License](../LICENSE); dependencies retain their own terms.
